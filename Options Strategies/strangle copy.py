...
req_list_=[{"Exch":"N","ExchType":"C","Symbol":"BANKNIFTY","Scripcode":"999920005","OptionType":"EQ"},
            {"Exch":"N","ExchType":"D","Symbol":"NIFTY 02 SEP 2021 PE 15500.00","Expiry":"20210902","StrikePrice":"15500","OptionType":"PE"}]          
...
x=a['Data'][0]['LastRate']
req_list_PE=[{"Exch":"N","ExchType":"D","Symbol":"BANKNIFTY 02 SEP 2021 PE "+str(int(x/100)*100)+".00","Expiry":"20210902","StrikePrice":str(int(x/100)*100),"OptionType":"PE"}]
req_list_CE=[{"Exch":"N","ExchType":"D","Symbol":"BANKNIFTY 02 SEP 2021 CE "+str(int(x/100)*100)+".00","Expiry":"20210902","StrikePrice":str(int(x/100)*100),"OptionType":"CE"}]
req_list_PE_strikeprice=[int(x/100)*100]
req_list_CE_strikeprice=[int(x/100)*100]
for i in range(1,25):
    req_list_PE=req_list_PE+[{"Exch":"N","ExchType":"D","Symbol":"BANKNIFTY 02 SEP 2021 PE "+str(int(x/100)*100-i*100)+".00","Expiry":"20210902","StrikePrice":str(int(x/100)*100-i*100),"OptionType":"PE"}] 
    req_list_PE_strikeprice=req_list_PE_strikeprice+[int(x/100)*100-i*100]
    req_list_CE=req_list_CE+[{"Exch":"N","ExchType":"D","Symbol":"BANKNIFTY 02 SEP 2021 CE "+str(int(x/100)*100+i*100)+".00","Expiry":"20210902","StrikePrice":str(int(x/100)*100+i*100),"OptionType":"CE"}] 
...
strategy.iron_condor("banknifty",[str(CE_hedge),str(PE_hedge)],[str(PE_lower),str(CE_upper)],'25','20210902','D')
...
if day!=0:
    ...
    for j in range(0,len(req_list_PE)):
        if req_list_PE[i]==Current_PE_strikeprice:
            PE_req=req_list_PE
            break
...
while True:
    ...
    if ce_lastrate>=2*pe_lastrate and CE_req['StrikePrice']!=PE_req['StrikePrice']:
        for i in range(0,len(positions)):
            if req_list_[1]['Symbol']==str.upper(positions[i]['ScripName']):
                ...
                scripcode_=str(int(script[script['Expiry']=='2021-09-02 14:30:00'][script['StrikeRate']==live_PE_lastrate[PE_index_strikeprice]][script['CpType']=='PE']['Scripcode']))
        ...
        for i in range(0,len(positions)):
            if req_list_[0]['Symbol']==str.upper(positions[i]['ScripName']):
                ...
                #sell pe which is 80 to 95% of ce
                scripcode_=str(int(script[script['Expiry']=='2021-09-02 14:30:00'][script['StrikeRate']==live_CE_lastrate[CE_index_strikeprice]][script['CpType']=='CE']['Scripcode']))
                test_order2=Order(order_type='S',exchange='N',exchange_segment='D', scrip_code=scripcode_, quantity=25,price=0,is_intraday=False,atmarket=True)
                Client.place_order(test_order2)
                CE_req = req_list_CE[CE_index_strikeprice]
                break
    if CE_req['StrikePrice']==PE_req['StrikePrice']:
        ...
        if Total_value_new>Stop_loss:
            ...
            test_order = Order(order_type='B',exchange='N',exchange_segment='D', scrip_code=str(int(script[script['FullName']=='BANKNIFTY 02 Sep 2021 PE '+CE_req['StrikePrice']+'.00'])), quantity=25,price=0,is_intraday=False,atmarket=True)
            Client.place_order(test_order)
            test_order = Order(order_type='B',exchange='N',exchange_segment='D', scrip_code=str(int(script[script['FullName']=='BANKNIFTY 02 Sep 2021 CE '+CE_req['StrikePrice']+'.00'])), quantity=25,price=0,is_intraday=False,atmarket=True)
            Client.place_order(test_order)
            test_order = Order(order_type='S',exchange='N',exchange_segment='D', scrip_code=str(int(script[script['FullName']=='BANKNIFTY 02 Sep 2021 CE '+str(CE_hedge)+'.00'])), quantity=25,price=0,is_intraday=False,atmarket=True)
            Client.place_order(test_order)
            test_order = Order(order_type='S',exchange='N',exchange_segment='D', scrip_code=str(int(script[script['FullName']=='BANKNIFTY 02 Sep 2021 PE '+str(PE_hedge)+'.00'])), quantity=25,price=0,is_intraday=False,atmarket=True)
            Client.place_order(test_order)
