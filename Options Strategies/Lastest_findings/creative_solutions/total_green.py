...
def total_green_counter_indicator(start,indicator,x,option_chain,strike,type,lastrate_taken,lots):
    if start==0:
        at_strike=int(np.round(x/100)*100)
        ce_data=option_chain[option_chain['CPType']=='CE']
        pe_data=option_chain[option_chain['CPType']=='PE']
        c_lastrate=float(ce_data[ce_data['StrikeRate']==at_strike]['LastRate'])
        p_lastrate=float(pe_data[pe_data['StrikeRate']==at_strike]['LastRate'])

        if indicator<0:
            index=np.argmin(np.abs(np.array(ce_data['LastRate']-c_lastrate/2)))
            c_strike=ce_data['StrikeRate'].iloc[index]
            order_button(at_strike,'CE_B',lots)
            order_button(c_strike,'CE_S',2*lots)
            start,strike,type,lastrate_taken=1,c_strike,'CE',c_lastrate/2
            return start,strike,type,lastrate_taken
        if indicator>0:
            index=np.argmin(np.abs(np.array(pe_data['LastRate']-p_lastrate/2)))
            p_strike=pe_data['StrikeRate'].iloc[index]
            order_button(at_strike,'PE_B',lots)
            order_button(p_strike,'PE_S',2*lots)
            start,strike,type,lastrate_taken=1,p_strike,'PE',p_lastrate/2
            return start,strike,type,lastrate_taken
    elif start==1:
        if type=='CE' and x>strike:
            order_button(strike,'CE_B',lots)
            order_button(strike,'PE_B',lots)
            pe_data=option_chain[option_chain['CPType']=='PE']
            ce_data=option_chain[option_chain['CPType']=='CE']
            c_lastrate=float(ce_data[ce_data['StrikeRate']==strike]['LastRate'])
            p_lastrate=float(pe_data[pe_data['StrikeRate']==strike]['LastRate'])
            diff=c_lastrate-lastrate_taken 
            index=np.argmin(np.abs(np.array(pe_data['LastRate']-(diff/2)-(p_lastrate/2))))
            p_strike=pe_data['StrikeRate'].iloc[index]
            order_button(p_strike,'PE_S',2*lots)
            start,strike,type,lastrate_taken=1,p_strike,'PE',(p_lastrate+diff)/2
            return start,strike,type,lastrate_taken
        if type=='PE' and x<strike:
            order_button(strike,'CE_B',lots)
            order_button(strike,'PE_B',lots)
            pe_data=option_chain[option_chain['CPType']=='PE']
            ce_data=option_chain[option_chain['CPType']=='CE']
            c_lastrate=float(ce_data[ce_data['StrikeRate']==strike]['LastRate'])
            p_lastrate=float(pe_data[pe_data['StrikeRate']==strike]['LastRate'])
            diff=p_lastrate-lastrate_taken 
            index=np.argmin(np.abs(np.array(ce_data['LastRate']-(diff/2)-(c_lastrate/2))))
            c_strike=ce_data['StrikeRate'].iloc[index]
            order_button(c_strike,'CE_S',2*lots)
            start,strike,type,lastrate_taken=1,c_strike,'CE',(c_lastrate+diff)/2
            return start,strike,type,lastrate_taken
...
if start==0:
    strike,type,lastrate_taken,lots=0,'',0,tron
...
while int(ind_time[11:13])*60+int(ind_time[14:16])<931:
    ...
    start,strike,type,lastrate_taken=total_green_counter_indicator(start,B,x,option_chain,strike,type,lastrate_taken,lots)
