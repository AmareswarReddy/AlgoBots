...
def initial_strangle_trades(option_chain,x,tron):
    ...
    factor=float(1)*int(np.ceil(f/100)*100)
...
def long_2n_adjustment(pstrike, cstrike, tron, option_chain, initial_premium_sum):
    ce_data = option_chain[option_chain['CPType'] == 'CE']
    pe_data = option_chain[option_chain['CPType'] == 'PE']
    ...
    return tron, initial_premium_sum
...
if to_take_from_positions=='y':
    ...
    initial_premium_sum=positions_record['initial_premium_sum']
...
while int(ind_time[11:13])*60+int(ind_time[14:16])<928 :
    ind_time = datetime.now(timezone("Asia/Kolkata")).strftime('%Y-%m-%d %H:%M:%S.%f')
    option_chain,x=data(week=week)
    check1[int(ind_time[11:13])*60+int(ind_time[14:16])+int(ind_time[17:19])/100]={'x':x,'opt':option_chain}
    print(x)
    print(int(ind_time[11:13])*60+int(ind_time[14:16])+int(ind_time[17:19])/100)
    ct,pt,et=c_strike,p_strike,exclusive_strike
    ...
    shoot,overnight_exclusive_strike,ptron,ctron=0,0,0,0
...
positions_json={'strangle':{'call_strike':c_strike,'put_strike':p_strike,'tron':tron},
                ...
                'initial_premium_sum':initial_premium_sum}
