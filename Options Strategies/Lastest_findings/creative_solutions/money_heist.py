...
def initial_strangle_trades(option_chain, x, tron):
    ...
    if tron == 0:
        tron = 1 #int(prime_client['login'].margin()[0]['AvailableMargin']/400000)
...
def initial_leg_trades(x, option_chain, tron):
    ...
    factor = 200  # max(100, int(np.floor((f)/100)*100))
    c_strike = exclusive_strike+factor
    p_strike = exclusive_strike-factor
    k, y1 = order_button(p_strike, 'PE_B', tron)
    while True:
...
def surya(x, option_chain, c_strike_b, p_strike_b, c_leg_tron, p_leg_tron, exclusive_strike, strangle_c_strike, strangle_p_strike, strangle_tron, initial_leg_tron):
    if strangle_tron > 0:
        ...
        if x > c_strike_b and c_lastrate > 100:
            ...
            order_button(
                c_strike_b-call_factor, 'CE_B', c_leg_tron)
            while y != 0:
                if strangle_tron == 0:
                    break
            ...
            while y != 0:
                ...
                new_p_strike_b, y = order_button(
                    p_strike_b-put_factor, 'PE_B', p_leg_tron+extra_lots)
            o, y = order_button(p_strike_b, 'PE_S', 2*p_leg_tron+extra_lots)
            while y != 0:
                if strangle_tron == 0:
                    break
                order_button(strangle_c_strike, 'CE_B', 1)
                ...
                strangle_tron -= 1
                o, y = order_button(p_strike_b, 'PE_S', 2*p_leg_tron+extra_lots)
            p_leg_tron=p_leg_tron*2+ extra_lots
...
if start == 0:
    ...
    tron = int(prime_client['login'].margin()[0]['AvailableMargin']/200000)
