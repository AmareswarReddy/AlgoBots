...
#list_of_files=['2023-03-29WEEKLY-expiry_data_BNF_Options.csv']

f=[]
for a in list_of_files:
    if 'WEEK' not in a.upper():
        f+=[a]
        ...
        if True:
            ...
            def triple_indicator(option_chain,x,cv,pv,coi,poi,day_cv,day_pv,day_coi,day_poi,earlier_coi,earlier_poi):
                ...
                try:
                    ...
                    a,b=pso(func=loss_function,lb=[end],ub=[i],minfunc=0.1)
            ...
            def data_modifier(filename):
                ...
                #df_banknifty_options = df_banknifty[(df_banknifty['Ticker'].str.contains('BANKNIFTY'))&(df_banknifty['Ticker'].str[-1]=='E')&(df_banknifty['Ticker'].str.contains('00'))]
                df_banknifty_options = df_banknifty[(df_banknifty['Ticker'].str.contains('BANKNIFTY'))&(df_banknifty['Ticker'].str[-1]=='E')&(df_banknifty['Ticker'].str.contains('00'))]
                ...
                try:
                    ...
                    df_banknifty_options['date_only'] = df_banknifty_options['Date/Time'].apply(lambda date:str(datetime.strptime(date, "%Y-%m-%d %H:%M:%S").date()))
                ...
                for group in df_banknifty_grouped_by_time.groups:
                    ...
                    try:
                        ...
                        df['seconds'] = df['Date/Time'].apply(lambda date : (datetime.strptime(date, "%Y-%m-%d %H:%M:%S") - start_time).seconds)
                    ...
                    try:
                        ...
                        df['StrikeRate'] = df[['Ticker', 'CPType']].apply(lambda x:int(x[0].split("Y")[1].split(x[1])[0]), axis=1)
