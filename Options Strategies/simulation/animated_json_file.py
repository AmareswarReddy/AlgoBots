...
with open('variables_data_30.json', 'r') as  json_file:
...
for i in range(2,len(k)):
    area=area+[np.sum(k[i-120:i])/len(k[:i])]
    if k[i]-k[i-1]>0.4 and k[i]<-tempo and taken_trade==0 :
        ...
        for kk in range(0,min(5,length)):
            ...
            print((lastrate[i]-p1[kk])*max(1,np.power(2,max(0,kk-1))))
        print('--------------------------------------------',i)
        if length>max_len:
            max_len=length
        #print(p1)
...
#ax_right.plot(area, color='red')
ax_right.plot(np.array(niftybank), color='green')
plt.show()

# %%
for i in range(220,len(lastrate)):
