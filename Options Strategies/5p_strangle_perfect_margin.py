...
def new_short_straddle(main_str_format_pe,main_str_format_ce,main_str_pe,main_str_ce,pe_mem,ce_mem):  #do not try running this function seperately. this is just an add on to strangle. 
    ...
    while True:
        ...
        for i in range(0,lots):
            strategy.adjustment_ce('banknifty',[str(ce_memory)],[str(req_list_CE_strikeprice)],'25',expiry,'D')
            strategy.adjustment_pe('banknifty',[str(pe_memory)],[str(req_list_PE_strikeprice)],'25',expiry,'D')
...
lots = int(np.floor(money_in_account))
...
while True:
    ...
    if x<int(pe_mem) and money_heist==0:
        ...
        for i in range(0,lots):
            strategy.adjustment_ce('banknifty',[str(ce_mem)],[str(pe_mem)],'25',expiry,'D')
        ind_time = datetime.now(timezone("Asia/Kolkata")).strftime('%Y-%m-%d %H:%M:%S.%f')
        ...
        log_response('pe_lastrate: '+str(pe_lastrate))
        for i in range(0,lots):
            strategy.adjustment_pe('banknifty',[str(pe_mem)],[str(ce_mem)],'25',expiry,'D')
        ind_time = datetime.now(timezone("Asia/Kolkata")).strftime('%Y-%m-%d %H:%M:%S.%f')
    ...
    if ce_lastrate>2*pe_lastrate and int(CE_req['StrikePrice'])-int(PE_req['StrikePrice'])>0:
        ...
        for i in range(0,lots):
            strategy.adjustment_pe('banknifty',[str(pe_mem)],[str(req_list_PE_strikeprice[PE_index_strikeprice])],'25',expiry,'D')
        ind_time = datetime.now(timezone("Asia/Kolkata")).strftime('%Y-%m-%d %H:%M:%S.%f')
        log_response(ind_time)
        ...
        for i in range(0,lots):
            strategy.adjustment_ce('banknifty',[str(ce_mem)],[str(req_list_CE_strikeprice[CE_index_strikeprice])],'25',expiry,'D')
        ind_time = datetime.now(timezone("Asia/Kolkata")).strftime('%Y-%m-%d %H:%M:%S.%f')
        log_response(ind_time)
