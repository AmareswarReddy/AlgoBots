...
start_cpLTP=75
days_in_strategy=6
vix1=pd.read_csv('hist_india_vix_01-Jan-2020_30-Dec-2020.csv')
vix2=pd.read_csv('hist_india_vix_01-Jan-2021_16-Dec-2021.csv')
vix=pd.concat([vix1,vix2])
vix.set_index("Date ", inplace = True)
def startPrice(vix=vix,fromdate='2021-12-10'):
    temp=int(fromDate[5:7])
    month={1:'Jan',
                    2: 'Feb',
                    3: 'Mar',
                    4: 'Apr',
                    ...
                    12: 'Dec'}
    try:
        value= vix.loc[fromDate[-2:]+'-'+month[temp]+'-'+fromDate[:4]]['Open ']
    except Exception:
        try:
            value= vix.loc[str(int(fromDate[-2:])+1)+'-'+month[temp]+'-'+fromDate[:4]]['Open ']
        except Exception:
            try:
                value= vix.loc[str(int(fromDate[-2:])-1)+'-'+month[temp]+'-'+fromDate[:4]]['Open ']
            except Exception:
    ...
    # 11   :  200 
    # 30   :  25
    startvalue=min(110,max(10000*np.power(1.25,-value),80))
    return startvalue
...
start_cpLTP_list= [75, 115,0]
df_writeData = pd.DataFrame(columns=['Expiry', 'Total Profit Booked 75', 'Total Profit Booked 115','Total_profit(vix boosted start)','start price used'])

for i in range(0, len(expires_list)):
    
    expiry = expires_list[i]
    if '2020' in expires_list[i]:
        profit_75 = 0
        profit_115 = 0
        profit_vix_adjusted = 0
        enddate = datetime.strptime(expiry, '%d%b%Y')
        startdate = enddate - timedelta(days=days_in_strategy)
        fromDate = startdate.strftime('%Y-%m-%d')
        start_cpLTP_list[-1]=startPrice(vix,fromdate=fromDate)
        for price in start_cpLTP_list:
            ...
            #
                if Current_CE_strikeprice-Current_PE_strikeprice<=gamma:
                    ...
                    if Total_value_new<Total_value_old:
                        Stop_loss=Total_value_new*delta
            ...
            if start_cpLTP == 75:
                ...
                profit_vix_adjusted = profit[-1]
        df_writeData.loc[0 if pd.isnull(df_writeData.index.max()) else df_writeData.index.max() + 1] = [expiry,profit_75, profit_115,profit_vix_adjusted,start_cpLTP_list[-1] ]
